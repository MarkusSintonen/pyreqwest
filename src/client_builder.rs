...
impl ClientBuilder {
    ...
    fn default_headers(slf: PyRefMut<Self>, headers: HeaderMapExt) -> PyResult<PyRefMut<Self>> {
    ...
    fn proxy<'py>(slf: PyRefMut<'py, Self>, proxy: Bound<'py, ProxyConfig>) -> PyResult<PyRefMut<'py, Self>> {
        Self::apply(slf, |builder| Ok(builder.proxy(proxy.get().build_reqwest_proxy()?)))
    }
