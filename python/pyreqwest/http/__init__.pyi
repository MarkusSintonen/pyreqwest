from collections.abc import MutableMapping, ItemsView, KeysView, ValuesView
from types import MappingProxyType
from typing import Sequence, Mapping, Iterable, Iterator, overload, TypeVar
from typing_extensions import Self

from ..bytes import Bytes
from .types import Stream, Params

T = TypeVar("T")


class Body:
    @staticmethod
    def from_text(body: str) -> "Body": ...
    @staticmethod
    def from_bytes(body: bytes | bytearray | memoryview) -> "Body": ...
    @staticmethod
    def from_stream(async_gen: Stream) -> "Body": ...
    def copy_bytes(self) -> Bytes | None: ...
    def get_stream(self) -> Stream | None: ...
    def copy(self) -> Self: ...
    def __copy__(self) -> Self: ...


class Url:
    def __init__(self, url: str | Url) -> None: ...
    @staticmethod
    def parse(url: str) -> "Url": ...
    @staticmethod
    def parse_with_params(url: str, params: Params) -> "Url": ...
    def join(self, join_input: str) -> Self: ...
    def make_relative(self, base: Self) -> str | None: ...
    @property
    def origin_ascii(self) -> str: ...
    @property
    def origin_unicode(self) -> str: ...
    @property
    def scheme(self) -> str: ...
    @property
    def is_special(self) -> bool: ...
    @property
    def has_authority(self) -> bool: ...
    @property
    def authority(self) -> str: ...
    @property
    def cannot_be_a_base(self) -> bool: ...
    @property
    def username(self) -> str: ...
    @property
    def password(self) -> str | None: ...
    @property
    def has_host(self) -> bool: ...
    @property
    def host_str(self) -> str | None: ...
    @property
    def domain(self) -> str | None: ...
    @property
    def port(self) -> int | None: ...
    @property
    def port_or_known_default(self) -> int | None: ...
    @property
    def path(self) -> str: ...
    @property
    def path_segments(self) -> list[str] | None: ...
    @property
    def query_string(self) -> str | None: ...
    @property
    def query_pairs(self) -> list[tuple[str, str]]: ...
    @property
    def query_dict_multi_value(self) -> MappingProxyType[str, Sequence[str]]: ...
    @property
    def fragment(self) -> str | None: ...
    def with_fragment(self, fragment: str | None) -> Self: ...
    def with_query(self, query: Params) -> Self: ...
    def extend_query(self, query: Params) -> Self: ...
    def with_query_string(self, query: str | None) -> Self: ...
    def with_path(self, path: str) -> Self: ...
    def with_path_segments(self, segments: list[str]) -> Self: ...
    def with_port(self, port: int | None) -> Self: ...
    def with_host(self, host: str | None) -> Self: ...
    def with_ip_host(self, addr: str) -> Self: ...
    def with_username(self, username: str) -> Self: ...
    def with_password(self, password: str | None) -> Self: ...
    def with_scheme(self, scheme: str) -> Self: ...
    def copy(self) -> Self: ...
    def __copy__(self) -> Self: ...
    def __truediv__(self, join_input: str) -> Self: ...
    def __str__(self) -> str: ...
    def __repr__(self) -> str: ...
    def __hash__(self) -> int: ...
    def __richcmp__(self, other: Self, op: int) -> bool: ...


class Mime:
    @staticmethod
    def parse(mime: str) -> "Mime": ...
    @property
    def type_(self) -> str: ...
    @property
    def subtype(self) -> str: ...
    @property
    def suffix(self) -> str | None: ...
    @property
    def essence_str(self) -> str: ...
    def get_param(self, name: str) -> str | None: ...
    def params(self) -> list[tuple[str, str]]: ...
    def __copy__(self) -> Self: ...
    def __str__(self) -> str: ...
    def __repr__(self) -> str: ...
    def __hash__(self) -> int: ...
    def __richcmp__(self, other: Self, op) -> bool: ...


class HeaderMapItemsView(ItemsView[str, str]): ...
class HeaderMapKeyView(KeysView[str]): ...
class HeaderMapValuesView(ValuesView[str]): ...


class HeaderMap(MutableMapping[str, str]):
    def __init__(self, other: Mapping[str, str] | Iterable[tuple[str, str]] | None = None) -> None: ...
    def __len__(self) -> int: ...
    def __iter__(self) -> Iterator[str]: ...
    def __getitem__(self, key: str, /) -> str: ...
    def __setitem__(self, key: str, value: str, /) -> None: ...
    def __delitem__(self, key: str, /) -> None: ...
    def items(self) -> HeaderMapItemsView: ...
    def keys(self) -> HeaderMapKeyView: ...
    def values(self) -> HeaderMapValuesView: ...
    def total_len(self) -> int: ...
    def keys_len(self) -> int: ...
    def get_one(self, key: str) -> str | None: ...
    def get_all(self, key: str) -> list[str]: ...
    def insert(self, key: str, value: str) -> str | None: ...
    def append(self, key: str, value: str) -> bool: ...
    def remove(self, key: str) -> str | None: ...
    @overload
    def pop_all(self, key: str) -> list[str]: ...
    @overload
    def pop_all(self, key: str, /, default: T) -> list[str] | T: ...
    def extend(self, other: Mapping[str, str] | Iterable[tuple[str, str]]) -> None: ...
    def dict_multi_value(self) -> dict[str, list[str]]: ...
    def copy(self) -> Self: ...
    def __copy__(self) -> Self: ...
    def __str__(self) -> str: ...
    def __repr__(self) -> str: ...
